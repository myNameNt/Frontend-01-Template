window.onload = function () {
  ...
  function enableGesture (body) {
    ...
    let end = (point, context) => {
      ...
      if (context.isTap) {
        body.dispatchEvent(new CustomEvent('tap', { name: 'tap' }))
        console.log('tapEnd')
      }
      if (context.isPress) {
        body.dispatchEvent(new CustomEvent('pressend', { name: 'tap' }))
      }
      clearTimeout(context.timeHandle)
    }
    let cancel = (point, context) => {
      body.dispatchEvent(new CustomEvent('canceled', { name: 'tap' }))
      clearTimeout(context.timeHandle)
    }
  }
